...
function isInObjectPrototype(node) {
  ...
  return (
    ...
    node.parent.parent.left === 'MemberExpression' &&
    node.parent.parent.left.object.type === 'Identifier' &&
    isDefinedAsFunction(
      node.parent.parent.left.object.name, node.parent.parent.parent
    ) &&
    node.parent.parent.left.property.type === 'identifier' &&
    node.parent.parent.left.property.name === 'prototype'
  );
...
function isFunctionConstructor(node) {
  ...
  var prototypes = findAllPrototypeAssignments(node).filter(function (assignment) {
    return (
      assignment.left.object.type === 'MemberExpression' &&
